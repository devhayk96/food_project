        ...
        <v-card height="100%" width="100%">
            ...
            <v-form ref="form" v-else>
                <v-card-title class="text-capitalize">
                    ...
                    {{ $route.query.action }} Product Subgroup
                </v-card-title>
                    ...
                    <v-row>
                        ...
                        <v-col cols="12" md="12">
                            <v-select
                                ...
                                :items="_.map(productGroupList, ((cat)=>{ return {text: cat.name,value: cat.id}}))"
                                v-model="dataDetails.product_group_id"
                                :rules="textRules"
                                required
                            ></v-select>
                        </v-col>
    ...
    export default {
        ...
        data() {
            return {
                ...
                savePermission: 'product_subgroups-create'
            }
        },
        created() {
        ...
        mounted() {
            if(this.$route.query.action === 'edit'){
                this.savePermission = 'product_subgroups-update';
                this.fetchDetails();
            }
            this.fetchProductGroupList();
        },
        ...
        methods: {
            ...
            fetchProductGroupList: function () {
                const _this = this;
                axios.get('/web/v1/group')
                    .then(function (response) {
                        console.log(response);
                        _this.productGroupList = response.data.data;
                    })
            ...
            fetchDetails: function () {
                const _this = this;
                axios.get('/web/v1/subgroup/'+this.$route.query.id)
                    .then(function (response) {
                        console.log(response);
                        _this.dataDetails = response.data;
                        _this.dataDetails.product_group_id = _this.dataDetails.product_groups.id;
                    })
                    .catch(function (error) {
                        console.log(error);
                        _this.showError({message:"Error : ", error: error});
            ...
            addData: function () {
                ...
                axios.post('/web/v1/subgroup', formData)
                    .then(function (response) {
                        console.log(response);
                        if(response.data) {
                            ...
                            _this.$router.push({name:'product-subgroup-list'});
                        }
                        else{
            ...
            updateData: function () {
                ...
                axios.post('/web/v1/subgroup/' + this.dataDetails.id, formData)
                    .then(function (response) {
                        console.log(response);
                        if(response.data) {
                            _this.showSuccess('Data is updated.');
